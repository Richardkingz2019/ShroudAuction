/**
 * Deploy the ShroudAuction contract to a Midnight network (undeployed by
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy ShroudAuction to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. The deployer is the auctioneer and never bids, so
      // the placeholder private state below is never opened against a
      // commitment. args is the contract constructor's arguments: empty for
      // ShroudAuction's no-arg constructor. (Statically-typed contracts can omit
      ...
      deployed = await deployContract(providers, {
        ...
        initialPrivateState: createAuctioneerPrivateState(),
      });
      break;
    } catch (err: any) {
